...
try:
    ...
    if(dbcon):
        ...
        if(cursor.execute(existsquery)==0):
            ...
            query1="select userid from tbl_register where username='%s' and usertype='%s'"%(sessname,sesstype)
            cursor.execute(query1)
            getdata1=cursor.fetchone()
            userid=getdata1[0]
        print("""
        ...
        print("""</ul>
                        ...
                        <h2>File Downloaded by %s</h2>"""%(sessname))
        print("""<br>
        ...
        query2="select fileid,userid,username,filename,downloadedon from tbl_download where userid=%d and username='%s'"%(int(userid),sessname)
        ...
        for row in result:
            ...
            print("<tr><td>%d</td><td>%s</td><td>%s</td><td>%s</td></tr>"%(int(fileid),ownername,filename,downloadedon))
