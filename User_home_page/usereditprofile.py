...
try:
    ...
    if(dbcon):
        ...
        if(res==1):
            dbcon.commit()
            print("<script>alert('Updated success');location.href='userhomepage.py?sname=%s&stype=%s';</script>"%(oname,'User'))
        else:
            dbcon.rollback()
            print("<script>alert('Error in Updation');location.href='userhomepage.py?sname=%s&stype=%s';</script>"%(oname,'User'))
    else:
        print("<script>alert('DB Error');location.href='userhomepage.py?sname=%s&stype=%s';</script>"%(oname,'User'))
