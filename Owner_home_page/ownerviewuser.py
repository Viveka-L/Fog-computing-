...
try:
    ...
    if(dbcon):
        ...
        query2="select fileid,userid,username,filename,downloadedon from tbl_download where ownerid=%d and fileowner='%s'"%(int(userid),sessname)
        ...
        for row in result:
            ...
            filename=row[3]
            downloadedon=row[4]
            print("<tr><td>%d</td><td>%d</td><td>%s</td><td>%s</td><td>%s</td></tr>"%(int(fileid),int(uid),username,filename,downloadedon))
